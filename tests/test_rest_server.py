...
@pytest.mark.asyncio
async def test_bundles_actions_reset_work_priority(mongo, rest):
    """Check reset work priority action for bundles."""
    r = rest('system')
    ...
    #
    # Read - GET /Bundles/UUID
    # Update - PATCH /Bundles/UUID
    #
    for result in results:
        ret = await r.request('GET', f'/Bundles/{result}')
        assert ret["uuid"] == result
        assert ret["create_timestamp"]
        assert ret["work_priority_timestamp"]

        await r.request('PATCH', f'/Bundles/{result}', {
    ...
    for result in results:
        ...
        assert ret["uuid"] == result
        assert ret["create_timestamp"]
        assert not ret["work_priority_timestamp"]

    #
    # Update - POST /Bundles/actions/reset_work_priority
    #
    await r.request('POST', '/Bundles/actions/reset_work_priority')
    ...
    for result in results:
        ...
        assert ret["create_timestamp"]
        assert ret["work_priority_timestamp"]
        assert ret["create_timestamp"] == ret["work_priority_timestamp"]

@pytest.mark.asyncio
async def test_requests_actions_reset_work_priority(mongo, rest):
    """Check reset work priority action for requests."""
    r = rest('system')
    ...
    #
    # Create - POST /TransferRequests/actions/bulk_create
    #
    await r.request('POST', '/TransferRequests', test_data)

    #
    # Read - GET /TransferRequests
    #
    ret = await r.request('GET', '/TransferRequests')
    results = ret["results"]
    assert len(results) == 1
    ...
    #
    # Read - GET /TransferRequests/UUID
    # Update - PATCH /TransferRequests/UUID
    #
    for re in results:
        result = re["uuid"]
        ret = await r.request('GET', f'/TransferRequests/{result}')
        assert ret["uuid"] == result
        assert ret["create_timestamp"]
        assert ret["work_priority_timestamp"]

        await r.request('PATCH', f'/TransferRequests/{result}', {
            "work_priority_timestamp": None
        })
    ...
    #
    # Update - POST /TransferRequests/actions/reset_work_priority
    #
    await r.request('POST', '/TransferRequests/actions/reset_work_priority')
