...
async def rest(
    monkeypatch: MonkeyPatch, port: int
) -> AsyncGenerator[RestClientFactory, None]:
    ...
    def client(role: str = "admin", timeout: float = 0.5) -> RestClient:
        ...
        token_data: Dict[str, Any] = {
            "resource_access": {"long-term-archive": {"roles": [role]}}
        }
        logging.info("setting role to %s", role)
        token = auth.create_token(
            subject="lta",  # type: ignore[no-untyped-call]
            expiration=300,
            payload=token_data,
            headers=None,
        )
        return RestClient(
            f"http://localhost:{port}", token=token, timeout=timeout, retries=0
        )
...
@pytest.mark.asyncio
async def test_transfer_request_crud(
    mongo: LtaCollection, rest: RestClientFactory
) -> None:
...
async def test_transfer_request_pop(rest: RestClientFactory) -> None:
    ...
    ret = await r.request(
        "POST", "/TransferRequests/actions/pop?source=NERSC", nersc_pop_claimant
    )
    ...
    ret = await r.request(
        "POST", "/TransferRequests/actions/pop?source=WIPAC", wipac_pop_claimant
    )
...
@pytest.mark.asyncio
async def test_get_bundles_filter(
    mongo: LtaCollection, rest: RestClientFactory
) -> None:
...
@pytest.mark.asyncio
async def test_get_bundles_request_filter(
    mongo: LtaCollection, rest: RestClientFactory
) -> None:
    ...
    ret = await r.request(
        "GET", "/Bundles?request=dd162dad-9880-4ed7-b3c3-f8843d765ac3"
    )
    results = ret["results"]
    assert len(results) == 0

    ret = await r.request(
        "GET", "/Bundles?request=5aba93ec-3c7d-43d7-8fe9-c19e5bc25991"
    )
    results = ret["results"]
    assert len(results) == 1

    ret = await r.request(
        "GET", "/Bundles?request=baebf071-702f-4ab5-9486-a9dec5420b84"
    )
    results = ret["results"]
    assert len(results) == 2
    r.close()
...
@pytest.mark.asyncio
async def test_delete_bundles_uuid(
    mongo: LtaCollection, rest: RestClientFactory
) -> None:
...
@pytest.mark.asyncio
async def test_patch_bundles_uuid(
    mongo: LtaCollection, rest: RestClientFactory
) -> None:
...
@pytest.mark.asyncio
async def test_bundles_actions_pop(
    mongo: LtaCollection, rest: RestClientFactory
) -> None:
    ...
    ret = await r.request(
        "POST", "/Bundles/actions/pop?source=NERSC&status=inaccessible", claimant_body
    )
    assert not ret["bundle"]

    # I'm the bundler at WIPAC, and should pop one work item
    ret = await r.request(
        "POST", "/Bundles/actions/pop?source=WIPAC&status=inaccessible", claimant_body
    )
    assert ret["bundle"]
    assert (
        ret["bundle"]["path"]
        == "/tmp/path1/sub1/24814fa8-875b-4bae-b034-ea8885d2aafe.zip"
    )
    ...
    # I'm the bundler at WIPAC, and should pop one work item
    ret = await r.request(
        "POST", "/Bundles/actions/pop?source=WIPAC&status=accessible", claimant_body
    )
    assert ret["bundle"]
    assert (
        ret["bundle"]["path"]
        == "/data/exp/IceCube/2014/15f7a399-fe40-4337-bb7e-d68d2d28ec8e.zip"
    )
    r.close()


@pytest.mark.asyncio
async def test_bundles_actions_pop_errors(
    mongo: LtaCollection, rest: RestClientFactory
) -> None:
    ...
    with pytest.raises(Exception):
        await r.request(
            "POST",
            "/Bundles/actions/pop?source=WIPAC&status=supercalifragilisticexpialidocious",
            request,
        )

    with pytest.raises(Exception):
        await r.request(
            "POST", "/Bundles/actions/pop?source=WIPAC&status=none", request
        )
...
@pytest.mark.asyncio
async def test_bundles_actions_pop_at_destination(
    mongo: LtaCollection, rest: RestClientFactory
) -> None:
    ...
    ret = await r.request(
        "POST", "/Bundles/actions/pop?dest=NERSC&status=taping", claimant_body
    )
    assert ret["bundle"]
    assert (
        ret["bundle"]["path"]
...
@pytest.mark.asyncio
async def test_bundles_actions_bulk_create_huge(
    mongo: LtaCollection, rest: RestClientFactory
) -> None:
    """Check pop action for bundles at destination."""
    ...
    file_spec = {
        "_links": {
            ...
            "self": {"href": "/api/files/e73aab54-2ead-11ea-a750-f6a52f4853dd"},
        },
        "checksum": {
...
@pytest.mark.asyncio
async def test_metadata_delete_bundle_uuid(
    mongo: LtaCollection, rest: RestClientFactory
) -> None:
    ...
    request = {
        ...
        "files": [
            ...
            "864f0903-f207-478d-bac2-b437ebc07226",
        ],
    }
    ret = await r.request("POST", "/Metadata/actions/bulk_create", request)
    assert len(ret["metadata"]) == 3
...
@pytest.mark.asyncio
async def test_metadata_single_record(
    mongo: LtaCollection, rest: RestClientFactory
) -> None:
    """Check CRUD semantics for metadata."""
    r = rest("system")  # type: ignore[call-arg]
...
@pytest.mark.asyncio
async def test_metadata_bulk_crud(
    mongo: LtaCollection, rest: RestClientFactory
) -> None:
    """Check CRUD semantics for metadata."""
    r = rest("system")  # type: ignore[call-arg]
