...
def test_bind_setup_curl() -> None:
    ...
    cb_func = bind_callback({"message": "Hello,"})
...
def test_constructor_config_poison_values(
...
@pytest.mark.asyncio
async def test_script_main_sync(
    config: TestConfig, mocker: MockerFixture, monkeypatch: MonkeyPatch
) -> None:
    """
...
async def test_desy_mirror_replicator_logs_configuration(mocker: MockerFixture) -> None:
    ...
    EXPECTED_LOGGER_CALLS = [
        call(
            "desy_mirror_replicator 'logme-testing-desy-mirror-replicator' is configured:"
        ),
        call("CI_TEST = TRUE"),
...
@pytest.mark.asyncio
async def test_desy_mirror_replicator_run(
    config: TestConfig, mocker: MockerFixture
) -> None:
...
@pytest.mark.asyncio
async def test_desy_mirror_replicator_run_exception(
    config: TestConfig, mocker: MockerFixture
) -> None:
...
@pytest.mark.asyncio
async def test_desy_mirror_replicator_do_work_no_results(
    config: TestConfig, mocker: MockerFixture
) -> None:
    """Test that _do_work goes on vacation when the LTA DB has no work."""
    logger_mock = mocker.MagicMock()
    dwc_mock = mocker.patch(
        "lta.desy_mirror_replicator.DesyMirrorReplicator._do_work_claim",
...
@pytest.mark.asyncio
async def test_desy_mirror_replicator_do_work_yes_results(
    config: TestConfig, mocker: MockerFixture
) -> None:
...
@pytest.mark.asyncio
async def test_desy_mirror_replicator_do_work_yes_then_die(
    config: TestConfig, mocker: MockerFixture
) -> None:
    """Test that _do_work keeps working until the LTA DB has no work."""
...
@pytest.mark.asyncio
async def test_desy_mirror_replicator_do_work_pop_exception(
    config: TestConfig, mocker: MockerFixture
) -> None:
    ...
    lta_rc_mock.request.assert_called_with(
        "POST",
        "/Bundles/actions/pop?source=WIPAC&dest=DESY&status=staged",
        {"claimant": f"{p.name}-{p.instance_uuid}"},
    )


@pytest.mark.asyncio
async def test_desy_mirror_replicator_do_work_claim_no_result(
    config: TestConfig, mocker: MockerFixture
) -> None:
    ...
    lta_rc_mock = AsyncMock()
    lta_rc_mock.request = AsyncMock()
    lta_rc_mock.request.side_effect = [{"bundle": None}]
    rbtds_mock = mocker.patch(
        "lta.desy_mirror_replicator.DesyMirrorReplicator._replicate_bundle_to_destination_site",
        new_callable=AsyncMock,
    )
    p = DesyMirrorReplicator(config, logger_mock)
...
@pytest.mark.asyncio
async def test_desy_mirror_replicator_do_work_claim_yes_result(
    config: TestConfig, mocker: MockerFixture
) -> None:
...
@pytest.mark.asyncio
async def test_desy_mirror_replicator_do_work_claim_write_bundle_raise_exception(
    config: TestConfig, mocker: MockerFixture
) -> None:
    ...
    lta_rc_mock.request.assert_called_with(
        "PATCH", "/Bundles/8f03a920-49d6-446b-811e-830e3f7942f5", mocker.ANY
    )
    rbtds_mock.assert_called_with(
        mocker.ANY, {"uuid": "8f03a920-49d6-446b-811e-830e3f7942f5", "status": "staged"}
    )


@pytest.mark.asyncio
async def test_desy_mirror_replicator_replicate_bundle_to_destination_site_raise_exception(
    config: TestConfig, mocker: MockerFixture
) -> None:
    ...
    sync_class_mock = mocker.patch(
        "lta.desy_mirror_replicator.Sync", new_callable=MagicMock
    )
    sync_class_mock.return_value = AsyncMock()
    sync_class_mock.return_value.put_path = AsyncMock()
    sync_class_mock.return_value.put_path.side_effect = Exception(
        "DESY system admins won lottery; nobody left to fix the problems"
    )
    p = DesyMirrorReplicator(config, logger_mock)
    await p._do_work_claim(lta_rc_mock)
    ...
    lta_rc_mock.request.assert_called_with(
        "PATCH", "/Bundles/398ca1ed-0178-4333-a323-8b9158c3dd88", mocker.ANY
    )


@pytest.mark.asyncio
async def test_desy_mirror_replicator_replicate_bundle_to_destination_site(
    config: TestConfig, mocker: MockerFixture
) -> None:
    """Test that _replicate_bundle_to_destination_site calls Sync.put_path()."""
    logger_mock = mocker.MagicMock()
