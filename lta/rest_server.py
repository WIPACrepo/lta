...
class BundlesActionsBulkCreateHandler(BaseLTAHandler):
    ...
    async def post(self) -> None:
        ...
        LOG.debug(f"MONGO-START: db.Bundles.insert_many(documents={req['bundles']})")
        ret = await self.db.Bundles.insert_many(documents=req["bundles"])
        LOG.debug("MONGO-END:   db.Bundles.insert_many(documents)")
        ...
        for x in req["bundles"]:
            uuid = x["uuid"]
            uuids.append(uuid)
            LOG.info(f"created Bundle {uuid}")
            prometheus_record_status_write(
                collection=BUNDLES,
...
class BundlesActionsBulkDeleteHandler(BaseLTAHandler):
    ...
    async def post(self) -> None:
        ...
        for uuid in req["bundles"]:
            query = {"uuid": uuid}
            LOG.debug(f"MONGO-START: db.Bundles.delete_one(filter={query})")
            ret = await self.db.Bundles.delete_one(filter=query)
            LOG.debug("MONGO-END:   db.Bundles.delete_one(filter)")
            if ret.deleted_count > 0:
                LOG.info(f"deleted Bundle {uuid}")
                results.append(uuid)
...
class BundlesActionsBulkUpdateHandler(BaseLTAHandler):
    ...
    async def post(self) -> None:
        ...
        for uuid in req["bundles"]:
            ...
            LOG.debug(f"MONGO-START: db.Bundles.update_one(filter={query}, update={update_doc})")
            ret = await self.db.Bundles.update_one(filter=query, update=update_doc)
            LOG.debug("MONGO-END:   db.Bundles.update_one(filter, update)")
            if ret.modified_count > 0:
                LOG.info(f"updated Bundle {uuid}")
                results.append(uuid)
...
class BundlesHandler(BaseLTAHandler):
    ...
    async def get(self) -> None:
        ...
        results = []
        LOG.debug(f"MONGO-START: db.Bundles.find(filter={query}, projection={projection})")
        async for row in self.db.Bundles.find(filter=query,
                                              projection=projection):
            results.append(row["uuid"])
        LOG.debug("MONGO-END*:   db.Bundles.find(filter, projection)")
...
class BundlesActionsPopHandler(BaseLTAHandler):
    ...
    async def post(self) -> None:
        ...
        LOG.debug(f"MONGO-START: db.Bundles.find_one_and_update(filter={find_query}, update={update_doc}, projection={REMOVE_ID}, sort={FIRST_IN_FIRST_OUT}, return_document={AFTER})")
        bundle = await sdb.find_one_and_update(filter=find_query,
                                               update=update_doc,
                                               projection=REMOVE_ID,
                                               sort=FIRST_IN_FIRST_OUT,
                                               return_document=AFTER)
        LOG.debug("MONGO-END:   db.Bundles.find_one_and_update(filter, update, projection, sort, return_document)")
        # return what we found to the caller
        if not bundle:
            LOG.info(f"Unclaimed Bundle with source {source} and status {status} does not exist.")
        else:
            LOG.info(f"Bundle {bundle['uuid']} claimed by {claimant}")
...
class BundlesSingleHandler(BaseLTAHandler):
    ...
    async def get(self, bundle_id: str) -> None:
        ...
        LOG.debug(f"MONGO-START: db.Bundles.find_one(filter={query}, projection={projection})")
        ret = await self.db.Bundles.find_one(filter=query, projection=projection)
        LOG.debug("MONGO-END:   db.Bundles.find_one(filter, projection)")
        if not ret:
    ...
    async def patch(self, bundle_id: str) -> None:
        ...
        # update
        LOG.debug(f"MONGO-START: db.Bundles.find_one_and_update(filter={query}, update={update_doc}, projection={REMOVE_ID}, return_document={AFTER})")
        ...
        LOG.debug("MONGO-END:   db.Bundles.find_one_and_update(filter, update, projection, return_document)")
        if not from_db:
            raise tornado.web.HTTPError(404, reason="not found")
        ...
        LOG.info(f"patched Bundle {bundle_id} with {req}")
        self.write(from_db)
    ...
    async def delete(self, bundle_id: str) -> None:
        ...
        LOG.debug("MONGO-END:   db.Bundles.delete_one(filter)")
        LOG.info(f"deleted Bundle {bundle_id}")
        self.set_status(204)
...
class MetadataActionsBulkCreateHandler(BaseLTAHandler):
    ...
    async def post(self) -> None:
        ...
        LOG.debug(f"MONGO-START: db.Metadata.insert_many(documents=[{len(documents)} documents])")
        ret = await self.db.Metadata.insert_many(documents=documents)
        LOG.debug("MONGO-END:   db.Metadata.insert_many(documents)")
        create_count = len(ret.inserted_ids)
        ...
        for x in documents:
            uuid = x["uuid"]
            uuids.append(uuid)
            LOG.info(f"created Metadata {uuid}")
...
class MetadataActionsBulkDeleteHandler(BaseLTAHandler):
    ...
    async def post(self) -> None:
        ...
        for i in range(slice_index, NUM_UUIDS, DELETE_CHUNK_SIZE):
            ...
            LOG.debug(f"MONGO-START: db.Metadata.delete_many(filter={len(delete_slice)} UUIDs)")
            ret = await self.db.Metadata.delete_many(filter=query)
            LOG.debug("MONGO-END:   db.Metadata.delete_many(filter)")
...
class MetadataHandler(BaseLTAHandler):
    ...
    async def get(self) -> None:
        ...
        results = []
        LOG.debug(f"MONGO-START: db.Metadata.find(filter={query}, projection={projection}, limit={limit}, skip={skip})")
        async for row in self.db.Metadata.find(filter=query,
                                               projection=projection,
                                               skip=skip,
                                               limit=limit):
            results.append(row)
        LOG.debug("MONGO-END*:   db.Metadata.find(filter, projection, limit, skip)")
    ...
    async def delete(self) -> None:
        ...
        LOG.debug(f"MONGO-START: db.Metadata.delete_many(filter={query})")
        await self.db.Metadata.delete_many(filter=query)
        LOG.debug("MONGO-END:   db.Metadata.delete_many(filter)")
        LOG.info(f"deleted all Metadata records for Bundle {bundle_uuid}")
        self.set_status(204)
...
class MetadataSingleHandler(BaseLTAHandler):
    ...
    async def get(self, metadata_id: str) -> None:
        ...
        LOG.debug(f"MONGO-START: db.Metadata.find_one(filter={query}, projection={projection})")
        ret = await self.db.Metadata.find_one(filter=query, projection=projection)
        LOG.debug("MONGO-END:   db.Metadata.find_one(filter, projection)")
        if not ret:
            raise tornado.web.HTTPError(404, reason="not found")
        self.write(ret)
    ...
    async def delete(self, metadata_id: str) -> None:
        ...
        query = {"uuid": metadata_id}
        LOG.debug(f"MONGO-START: db.Metadata.delete_one(filter={query})")
        await self.db.Metadata.delete_one(filter=query)
        LOG.debug("MONGO-END:   db.Metadata.delete_one(filter)")
        LOG.info(f"deleted Bundle {metadata_id}")
        self.set_status(204)
...
class TransferRequestsHandler(BaseLTAHandler):
    ...
    async def get(self) -> None:
        ...
        ret = []
        LOG.debug(f"MONGO-START: db.TransferRequests.find(filter={ALL_DOCUMENTS}, projection={REMOVE_ID})")
        async for row in self.db.TransferRequests.find(filter=ALL_DOCUMENTS,
                                                       projection=REMOVE_ID):
            ret.append(row)
        LOG.debug("MONGO-END*:  db.TransferRequests.find(filter, projection)")
    ...
    async def post(self) -> None:
        ...
        req['claimed'] = False
        LOG.debug(f"MONGO-START: db.TransferRequests.insert_one(document={req}")
        await self.db.TransferRequests.insert_one(document=req)
        LOG.debug("MONGO-END:   db.TransferRequests.insert_one(document)")
        LOG.info(f"created TransferRequest {req['uuid']}")
...
class TransferRequestSingleHandler(BaseLTAHandler):
    ...
    async def get(self, request_id: str) -> None:
        ...
        LOG.debug(f"MONGO-START: db.TransferRequests.find_one(filter={query}, projection={REMOVE_ID}")
        ret = await self.db.TransferRequests.find_one(filter=query, projection=REMOVE_ID)
        LOG.debug("MONGO-END:   db.TransferRequests.find_one(filter, projection)")
        if not ret:
            raise tornado.web.HTTPError(404, reason="not found")
        self.write(ret)
    ...
    async def patch(self, request_id: str) -> None:
        ...
        LOG.debug(f"MONGO-START: db.TransferRequests.find_one_and_update(filter={query}, update={update}, projection={REMOVE_ID}, return_document={AFTER}")
        ...
        LOG.debug("MONGO-END:   db.TransferRequests.find_one_and_update(filter, update, projection, return_document")
        if not from_db:
            raise tornado.web.HTTPError(404, reason="not found")

        LOG.info(f"patched TransferRequest {request_id} with {req}")
        if "status" in req:  # did the user update the status?
            prometheus_record_status_write(
                collection=TRANSFER_REQUESTS,
                new_status=req["status"],
    ...
    async def delete(self, request_id: str) -> None:
        ...
        query = {"uuid": request_id}
        LOG.debug(f"MONGO-START: db.TransferRequests.delete_one(filter={query})")
        await self.db.TransferRequests.delete_one(filter=query)
        LOG.debug("MONGO-END:   db.TransferRequests.delete_one(filter)")
        LOG.info(f"deleted TransferRequest {request_id}")
        self.set_status(204)
...
class TransferRequestActionsPopHandler(BaseLTAHandler):
    ...
    async def post(self) -> None:
        ...
        LOG.debug(f"MONGO-START: db.TransferRequests.find_one_and_update(filter={find_query}, update={update_doc}, projection={REMOVE_ID}, sort={FIRST_IN_FIRST_OUT}, return_document={AFTER})")
        tr = await sdtr.find_one_and_update(filter=find_query,
                                            update=update_doc,
                                            projection=REMOVE_ID,
                                            sort=FIRST_IN_FIRST_OUT,
                                            return_document=AFTER)
        LOG.debug("MONGO-END:   db.TransferRequests.find_one_and_update(filter, update, projection, sort, return_document)")
        # return what we found to the caller
        if not tr:
            LOG.info(f"Unclaimed TransferRequest with source {source} does not exist.")
        else:
            LOG.info(f"TransferRequest {tr['uuid']} claimed by {claimant}")
            prometheus_record_status_write(
                collection=TRANSFER_REQUESTS,
